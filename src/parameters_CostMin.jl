...
function getrice2010parameters_costmin(filename, negishi::Bool)
    p = Dict{Symbol,Any}()
    ...
    #p[:l] = getparam_timeseries(f, "B159:BI159", regions, T) # Level of population and labor in RICE spreadsheet
    p[:l] = getparam_timeseries(f, "B56:BI56", regions, T) # UN Pop numbers
    ...
    p[:savings] = .258*ones(60,12)

    # MIU (base case RICE2010)
    #p[:MIU] = getparam_timeseries(f, "B103:BI103", regions, T)
    p[:CPRICE] = zeros(60,12) #Going to be solved over anyways.
