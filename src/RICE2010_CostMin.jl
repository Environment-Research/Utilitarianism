...
include("parameters_CostMin.jl")


#Notice: calls components from a different folder

include("components_CostMin/climatedynamics_component.jl")
include("components_CostMin/co2cycle_component.jl")
include("components_CostMin/damages_component.jl")
include("components_CostMin/emissions_component.jl")
include("components_CostMin/grosseconomy_component.jl")
include("components_CostMin/neteconomy_component.jl")
include("components_CostMin/radiativeforcing_component.jl")
include("components_CostMin/slr_component.jl")
include("components_CostMin/slrdamages_component.jl")
include("components_CostMin/welfare_component.jl")

function constructrice_costmin(p)
    al = p[:al]
    l = p[:l]
    gama = p[:gama]
    dk = p[:dk]
    ...
    savings = p[:savings]
    #MIU = p[:MIU] Not a parameter in OnePrice. 
    CPRICE = p[:CPRICE] #NEW PARAMETER IN COSTMIN BUT NOT FLEXIBLE
    m = Model()
    ...
    setparameter(m, :emissions, :CPRICE, CPRICE)
    ...
    setparameter(m, :welfare, :rho, rho)
    ...
    connectparameter(m, :welfare, :temperature, :climatedynamics, :TATM)

    return m
end

function getrice_costmin(;datafile=joinpath(dirname(@__FILE__), "..", "data", "RICE_2010_base_000.xlsm"),Negishi=true)
    params = getrice2010parameters_costmin(datafile, Negishi)

    m = constructrice_costmin(params)
