...
@defcomp welfare begin
    ...
    alpha = Parameter(index=[time, regions]) #Part of Negishi weights... turned off in main runs.
...
function run_timestep(state::welfare, t::Int)
    ...
    for r in d.regions
        if p.elasmu[r]==1.
            ...
            v.PERIODU[t,r] = ((1./(1.-p.elasmu[r])) * (p.CPC[t,r])^(1.-p.elasmu[r])) * p.alpha[t,r]
        end
    end
    ...
    for r in d.regions
       if t != 60
            ...
            v.CEMUTOTPER[t,r] = v.PERIODU[t,r] * p.l[t,r] * p.rr[t,r] / (1. - ((p.rr[t-1,r] / (1. + p.rho[r])^10) / p.rr[t-1,r]))
        end
    end
