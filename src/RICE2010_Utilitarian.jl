...
include("parameters_Utilitarian.jl")
...
include("components_Utilitarian/radiativeforcing_component.jl")
include("components_Utilitarian/slr_component.jl")
include("components_Utilitarian/slrdamages_component.jl")
include("components_Utilitarian/welfare_component.jl")

function constructrice_Utilitarian(p)
    al = p[:al]
    l = p[:l]
    gama = p[:gama]
    dk = p[:dk]
...
function getrice_Utilitarian(;datafile=joinpath(dirname(@__FILE__), "..", "data", "RICE_2010_base_000.xlsm"), Negishi=true)
    params = getrice2010parameters_Utilitarian(datafile, Negishi)

    m = constructrice_Utilitarian(params)
