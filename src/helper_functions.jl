...
#              The objective function will take in a vector of global carbon tax values (uniform price) or regional
...
function construct_rice_objective(run_utilitarian::Bool, ρ::Float64, η::Float64, backstop_prices::Array{Float64,2}, remove_negishi::Bool)
    ...
    #--------------------------------------------------------------------------------------------------------
    # Create either a (i) uniform price or (ii) utilitarian objective function for this instance of RICE.
    #--------------------------------------------------------------------------------------------------------
    rice_objective = if run_utilitarian == false

        #---------------------------------------
        # Uniform price objective function.
        #---------------------------------------
...
#              uniform price (global carbon taxes) or utilitarian (regional carbon taxes) approach to find the
...
function optimize_rice(optimization_algorithm::Symbol, n_opt_periods::Int, stop_time::Int, tolerance::Float64, backstop_prices::Array{Float64,2}; run_utilitarian::Bool=true, ρ::Float64=0.008, η::Float64=1.5, remove_negishi::Bool=true)
    ...
    # Set objective function, upper bound, and number of optimzation objectives (will differ between uniform price and utilitarian approaches).
    ...
    # Create optimal decarbonization rates for all time periods (approach to do so will differ between uniform price and utilitarian).
    if run_utilitarian == false
    ...
    # Create optimal tax rates for all time periods (approach to do so will differ between uniform price and utilitarian).
    if run_utilitarian == false
...
function construct_fund_objective(run_utilitarian::Bool, ρ::Float64, η::Float64, welfare_year::Int, end_year::Int)
    ...
    #--------------------------------------------------------------------------------------------------------
    # Create either a (i) uniform price or (ii) utilitarian objective function for this instance of FUND.
    #--------------------------------------------------------------------------------------------------------
...
function optimize_fund(optimization_algorithm::Symbol, n_opt_periods::Int, stop_time::Int, tolerance::Float64; run_utilitarian::Bool=true, ρ::Float64=0.008, η::Float64=1.5, welfare_year::Int=2010, end_year::Int=2300)
    ...
    # Set number of optimzation objectives (will differ between uniform price and utilitarian approaches).
    if run_utilitarian == false
    ...
    # Calculate the full tax series for all time periods and regions (approach to do so will differ between uniform price and utilitarian).
    if run_utilitarian == false
