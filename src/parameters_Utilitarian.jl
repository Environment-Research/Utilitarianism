...
function getrice2010parameters_Utilitarian(filename, negishi)
    p = Dict{Symbol,Any}()

    T = 60  #how many discretized time periods there are
    p[:timesteps] = 1:T # Time periods 
    ...
    # Welfare and Scaling parameters
    # Multiplicative scaling coefficient
    # If negishi is on, use the weights from the RICE spreadsheet
    # if off, set weights and additive pieces to 1 (if multiplicative) or 0 (if additive.. this shouldn't matter though)
    if negishi==true
    ...
    p[:savings] = .258*ones(60,12)  # exogenously set savings rate

    # MIU (base case RICE2010): Dont end up using this series, its what the solver loops over.
    # Mimi needs it read in as a parameter. In the optimization function (optmizeRICE.jl) it replaces this series
    # until global welfare is maximized
