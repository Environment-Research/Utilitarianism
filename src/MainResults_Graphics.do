************************************************************************
* File to import csv files and clean into graphable results for IAM
************************************************************************
...
#delimit ;
twoway (connected E2005 year if $oldfiles == ., lc(sand) ms(i)) (connected E2005 year if $oldfiles == 1, lc(maroon) lp(shortdash) ms(i)) 
...
(line CPCGains year if $oldfiles ==. & region=="India" & year>2016 & year<$lastyearCPC , lc($Indiac ) lw(thick))
...
(line CPCGains year if $oldfiles ==1 & region=="India" & year>2016 & year<$lastyearCPC , lc($Indiac ) lw(thick))
...
(line MIU year if region=="India" & year<2080 & year>2010 & $oldfiles !=1, lcolor($Indiac) lwidth(thick))
...
#delimit ;
twoway (scatter CPCfrac logCPC if CPCfrac>0 & leg!=1, msymbol(i) mlabel(region) mlabpos(center) mlabcolor(eltblue) mlabsize(vsmall))
...
twoway (scatter CPCfrac2 logCPC if CPCfrac2>0 & leg!=1, msymbol(i) mlabel(region) mlabpos(center) mlabcolor(eltblue) mlabsize(vsmall))
...
xlab(,labsize(small))
text(1.4 0.12 "Many Benefit", placement(e) color(eltblue) size(small))
...
ysize(6) xsize(10) title("Versus Cost-Minimization")
name(TophalfFig3);
...
sort year
#delimit ;
twoway (connected temp year if $oldfiles == . & region=="Africa" & year<$lastyear & year>2015, lw(.6) lc(sand) ms(i)) 
(connected temp year if $oldfiles == 1 & region=="Africa" & year<$lastyear & year>2015, lc(maroon) lw(.6) lp(longdash) ms(i)),
 name(temperature) xtitle("") ytitle("Increase from Pre-Industrial (C)") 
 legend(col(2) ring(0) order(1 "$newway" 2 "$oldway")) 
 graphr(c(white) lc(white)) ylab(, nogrid angle(0) format(%3.1f))
 xlab(2020 "2020" 2050 "2050" 2100 "2100" 2150 "2150" 2200 "2200");
 *yline(2.35);
#delimit cr
