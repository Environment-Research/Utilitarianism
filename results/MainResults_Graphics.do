...
sort year
#delimit ;
twoway (connected temp year if $oldfiles == . & region=="Africa" & year<$lastyear & year>2015, lw(.6) lc(forest_green) ms(i)) 
(connected temp year if $oldfiles == 1 & region=="Africa" & year<$lastyear & year>2015, lc(maroon) lw(.6) lp(longdash) ms(i)),
 title("Temperature Changes:" "Utilitarianism vs. Cost-Minimization", c(black)) name(temperature) xtitle("") ytitle("Increase from Pre-Industrial(C)") 
